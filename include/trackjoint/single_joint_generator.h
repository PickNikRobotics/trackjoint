...
public:
  ...
  /** \brief Get bitmask for use in unit testing */
  std::bitset<8> GetForwardLimitCompBitmask();
...
private:
  ...
  // Bit masks for unit testing
  // Mark when each conditions is triggered.
  // A bit for each condition in ForwardLimitCompensation()
  std::bitset<8> forward_limit_comp_conditions_;
