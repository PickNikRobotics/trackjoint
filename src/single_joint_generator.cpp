...
SingleJointGenerator::SingleJointGenerator(double timestep, double desired_duration, double max_duration,
                                           ...
                                           const trackjoint::Limits& limits, int desired_num_waypoints,
...
ErrorCodeEnum SingleJointGenerator::ExtendTrajectoryDuration()
{
  // Clear previous results
  waypoints_ = JointTrajectory();
  int expected_num_waypoints = 1 + desired_duration_ / kTimestep;
...
int SingleJointGenerator::GetLastSuccessfulIndex()
{
...
Eigen::VectorXd SingleJointGenerator::Interpolate(Eigen::VectorXd& times)
{
  ...
  for (int index = 0; index < tao.size(); ++index)
  {
...
ErrorCodeEnum SingleJointGenerator::LimitCompensation(int* index_last_successful)
{
  ...
  *index_last_successful = waypoints_.positions.size();
  ...
  int num_waypoints = waypoints_.positions.size();
  ...
  for (int index = 1; index < (num_waypoints - 1); ++index)
  {
  ...
  for (int index = 1; index < (num_waypoints - 1); ++index)
  {
    ...
    {
      delta_v =
          std::copysign(1.0, waypoints_.velocities(index)) * kLimits.velocity_limit - waypoints_.velocities(index);
...
void SingleJointGenerator::RecordFailureTime(int current_index, int* index_last_successful)
{
...
bool SingleJointGenerator::VelocityCompensation(int limited_index, double excess_velocity)
{
  // The algorithm:
  ...
  // calculations.
  for (int index = limited_index; index > 2; --index)
  {
...
ErrorCodeEnum SingleJointGenerator::PredictTimeToReach()
{
  ...
  while ((index_last_successful_ < waypoints_.positions.size()) && (desired_duration_ < max_duration_))
  {
    ...
    int new_num_waypoints = floor(1 + desired_duration_ / kTimestep);
...
ErrorCodeEnum SingleJointGenerator::PositionVectorLimitLookAhead(int* index_last_successful)
{
