                                     ...
                                     Eigen::VectorXd* velocity_vector, Eigen::VectorXd* acceleration_vector)
{
  ...
  int new_vector_size = 1 + round(final_time / kDesiredTimestep);
  // Time downsampling:
  ...
  int num_elements_to_skip;

  // Total number of elements filled in the new vector
  int num_elements_filled_in_new_vector = 0;
  // Number of elements traversed in the upsampled vector
  int num_upsampled_elements_traversed = 0;
  for (int count = 1; num_elements_filled_in_new_vector < new_vector_size - 2; ++count)
  {
                                                 ...
                                                 const std::string& base_filepath, bool append_to_file) const
{
  ...
  for (size_t joint = 0; joint < output_trajectories.size(); ++joint)
  {
    ...
    for (int waypoint = 0; waypoint < output_trajectories.at(joint).positions.size(); ++waypoint)
    {
