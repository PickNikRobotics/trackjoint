...
std::vector<std::bitset<8>> TrajectoryGenerator::GetForwardLimitCompBitmasks()
{
  std::vector<std::bitset<8>> joint_bitsets;

  for (size_t joint = 0; joint < kNumDof; ++joint)
  {
    joint_bitsets.push_back(single_joint_generators_[joint].GetForwardLimitCompBitmask());
  }

  return joint_bitsets;
}
